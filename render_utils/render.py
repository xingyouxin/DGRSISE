...
def tex2map(tex):
    ...
    # rough = ((tex[:,5,:,:].clamp(-1,1) + 1) / 2) # ** 2.2 # xyx
...
def tex2map_wo_Gamma(tex): # xyx
    ...
    # normal    = th.stack((normal_x, normal_y, normal_z), 1)
    # normal    = normal.div(normal.norm(2.0, 1, keepdim=True))
    normal    = tex[:,3:6,:,:].clamp(-1,1)

    rough = ((tex[:,6,:,:].clamp(-0.3,1) + 1) / 2)
    rough = rough.clamp(min=eps).unsqueeze(1).expand(-1,3,-1,-1)

    if tex.shape[1] == 10:
        specular = ((tex[:,7:10,:,:].clamp(-1,1) + 1) / 2)
        return albedo, normal, rough, specular

    return albedo, normal, rough


def getTexPos(res, size, N, device): # xyx
    tmp = th.arange(res, dtype=th.float32).to(device)
    tmp = ((tmp + 0.5) / res - 0.5) * size
    y, x = th.meshgrid((tmp, tmp))
    pos = th.stack((x, -y, th.zeros_like(x)), 2).permute(2, 0, 1).unsqueeze(0).expand(N, -1, -1, -1)
    return pos

def generateDirectionMap_simplized(pos, pos_texture): # xyx
    vec = pos - pos_texture
    return vec / (vec.norm(2.0, 1, keepdim=True))

class Microfacet:
    def __init__(self, res, size, N, f0=0.04):
        self.res = res
        self.size = size
        self.f0 = f0
        self.eps = 1e-6
    ...
    def initGeometry(self):
        ...
        y, x = th.meshgrid((tmp, tmp))
        self.pos = th.stack((x, -y, th.zeros_like(x)), 2).permute(2,0,1).unsqueeze(0).expand(self.N,-1,-1,-1)
    ...
    def eval(self, textures, lightPos, cameraPos, light):
        ...
        # if textures.size(1) == 10: # xyx
            isSpecular = True
            # print('Render Specular\n')

        if isSpecular:
            albedo, normal, rough, specular = tex2map(textures)
            # albedo, normal, rough, specular = tex2map_wo_Gamma(textures) # xyx
        else:
        ...
        return img.clamp(0,1), l, v, h

class Microfacet_xyx:
    ...
    def GGX(self, cos_h, alpha):
        ...
        den = c2 * a2 + (1 - c2)
        # return a2 / (np.pi * den**2 + self.eps)
        return a2 / torch.clamp (np.pi * den**2 , min=1e-8) # xyx
    ...
    def Smith(self, n_dot_v, n_dot_l, alpha):
        ...
        # k = alpha * 0.5 + self.eps
        k = torch.clamp(alpha * 0.5, min=1e-8) # xyx
        return _G1(n_dot_v, k) * _G1(n_dot_l, k)
    ...
    def AdotB(self, a, b):
        # ab = (a*b).sum(1, keepdim=True).clamp(min=0).expand(-1,3,-1,-1)
        ab = (a*b).sum(1, keepdim=True).clamp(min=1e-8).expand(-1,3,-1,-1) # xyx
        return ab
    ...
    def eval(self, textures, lightPos, cameraPos, light):
        ...
        # cook-torrence brdf
        # f2 = D * F * G / (4 * n_dot_v * n_dot_l + self.eps)
        f2 = D * F * G / (4 * n_dot_v * n_dot_l + 1e-12) # xyx
...
def renderTex(fn_tex, res, size, lp, cp, L, fn_im):
    ...
    # textures, tex_res = png2tex_all(fn_tex) # xyx
